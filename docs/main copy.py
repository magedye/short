...
class SimpleUserResolver(UserResolver):
    ...
    def resolve_user(self, user_id: Optional[str] = None) -> User:
        ...
        return User(user_id=user_id or "default_user")
...
def create_request_context(
    ...
    return RequestContext(user_id=user.user_id, conversation_id=conversation_id)
...
async def ask_question(request: AskRequest, stream: bool = Query(False)) -> AskResponse:
    ...
    try:
        ...
        sql = None
        try:
            sql = agent.generate_sql(request.question, context=request_context)
            if not sql:
        ...
        try:
            if row_count > 0 and viz_tool_global:
                ...
                tool_ctx = ToolContext(
                    user=SimpleUserResolver().resolve_user(request_context.user_id),
                    conversation_id=conversation_id,
